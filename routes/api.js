...
//API for patient messages
    app.post('/sentpmessage', function(req, res) {
        ...
        {
            conn.query('INSERT INTO patient_messages(username, message, sent_time) VALUES(?,?,?)', [req.body.sender, req.body.message, req.body.date], function (error, results) {
                if (error)
                {
                    console.log(error);
                }
    ...
    app.get('/getpmessagebyid', function(req, res) {       
        conn.query('SELECT message, DATE_FORMAT(sent_time,  "%m/%d/%Y") as date, DATE_FORMAT(sent_time,  "%T") as time FROM patient_messages WHERE username = ? AND sent_time BETWEEN ? AND ? ORDER BY sent_time desc', [req.query.username, req.query.startdate, req.query.enddate], function (error, results) {
            if (error)
            {
                console.log(error);
            }
    ...
    app.get('/getpmessagebydate', function(req, res) {
        conn.query('SELECT message, DATE_FORMAT(sent_time,  "%m/%d/%Y") as date, DATE_FORMAT(sent_time,  "%T") as time FROM patient_messages WHERE lower(username) = lower(?)  AND sent_time BETWEEN  ? AND ? ORDER BY sent_time desc', [req.query.username, req.query.startdate, req.query.enddate], function (error, results) {
            if (error)
            {
                console.log(error);
            }
    ...
    app.get('/getchatmessages', function(req, res) {
        ...
        conn.query('select * from chat_messages where sender = ? OR receiver = ? OR receiver = ? OR sender = ? order by sent_time;',
        [req.query.sender, req.query.receiver, req.query.sender, req.query.receiver]
    ...
    app.post('/sendchatmessages', function(req, res) {
    ...
    app.get('/getallusers', function(req, res) {
        conn.query('select full_name, username from users;'
        , function (error, result) {
            if (error)
            {
                console.log(error);
