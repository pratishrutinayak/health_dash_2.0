angular.module("app", ['chart.js', 'ngRoute', 'ngCookies', 'ngSanitize'])
  ...
  .config(['ChartJsProvider', '$routeProvider', '$httpProvider', function (ChartJsProvider, $routeProvider, $httpProvider) {
    ...
    $routeProvider
        ...
        .when('/patientMessages', {
            templateUrl : 'views/partial/patientMessages.html'
        });
...
// Sort By table in the GUI (Pratishruti)
    $scope.propertyName = [];
    $scope.reverse = true;
 ...
 $scope.goalstDate = "";
  ...
  $scope.$on('usernamechange', function(event, args){
    ...
    // Sort By and filter (Pratishruti)
    $scope.propertyName = [];
    $scope.pagedRecords = [];
    $scope.reverse = true;
  ...
  $scope.$on('usernamechange', function(event, args){
...
}]).controller("patientMsgCtrl", ['$scope', '$http', '$timeout', '$filter', function ($scope, $http, $timeout, $filter) {
    ...
    $scope.gap = 5;
    $scope.sender = $scope.$parent.$parent.username.username;
  ...
  function getByPatientId(val){
    ...
    var endOfDayToday = new Date(today.getFullYear()
                           ...
                           ,23,59,59);
      $http({
            url: '/api/getpmessagebyid',
            method: 'GET',
            params: {username: val.username == null? val: val.username, startdate: new Date().toLocaleDateString().split('/')[2] + "-" + new Date().toLocaleDateString().split('/')[0] + "-" + new Date().toLocaleDateString().split('/')[1] + " " + "00:00:00", enddate: new Date().toLocaleDateString().split('/')[2] + "-" + new Date().toLocaleDateString().split('/')[0] + "-" + new Date().toLocaleDateString().split('/')[1] + " " + "23:59:59"}
        })
        .success(function(data) {
            console.log(data);
            var dat = new Date().toLocaleString().split(',')[0].split('/');
            ...
            for(var i = 0; i< data.length; i++)
            {
                $scope.messages.push({message: data[i].message, 
                    date: data[i].date,
                    time: data[i].time});
            }
            $scope.search();
  ...
  function searchPatientMessage(val){

            $http({
            url: '/api/getpmessagebydate',
            method: 'GET',
            params: {username: $scope.$parent.$parent.username.username, 
                startdate: new Date(val.stDate).toLocaleDateString().split('/')[2] + "-" 
                        + new Date(val.stDate).toLocaleDateString().split('/')[0] + "-" 
                        + new Date(val.stDate).toLocaleDateString().split('/')[1] + " " 
                        + "00:00:00", 
  ...
  $scope.$on('usernamechange', function(event, args){
        getByPatientId(args.val);
        $scope.sender = args.val.username;
    });
    ...
    //call the search API on event searchByDate
    $scope.$on('searchbydate', function(event, args){
        searchPatientMessage(args);
    });
    //end
  
}]).controller('messaging',function($scope,$filter,$http,$cookies){
    $scope.username = "";
    $scope.messages = [];
    ...
    $scope.refresh = function(user){
        $scope.searchedName = "";
        $scope.username = user; // The person receiving the messages
        chat();
    }
    var chat = function(){
        ...
        $http({
            ...
            method:'GET',
            params:{sender:$scope.username, receiver:$scope.user}
        })
        .success(function(data){
            if(data.length){
        ...
        // Temporary selection of username

        $scope.sendMessage = function(){
            $http({
                url:"/api/sendchatmessages",
                method:'POST',
                params:{message:$scope.message,receiver:$scope.username,sender:$scope.user}
            })
            .success(function(data){
                    // $scope.messages = data;
                    $scope.message = '';
